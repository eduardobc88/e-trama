...
class Model extends Base {
  ...
  constructor(attributes = {}, collection = null, options = {}) {
    ...
    // NOTE: custom: added: generate empty errors
  ...
  /**
   * Generate an empty errors by avoid errors in some libraries like vuei18n
   */
  setDefaultErrors() {
    for (let attribute of Object.keys(this.attributes))
      this.setAttributeErrors(attribute, [])
  }
  ...
  set(attribute, value) {
    ...
    if (changed) {
      ...
      if (this.getOption('validateOnChange')) {
        nextTick(() => this.validateAttribute(attribute));
      }
  ...
  setAttributeErrors(attribute, errors) {
    if (isEmpty(errors)) {
      // delete this._errors[attribute] // NOTE: custom: original
      this._errors[attribute] = castArray('') // NOTE: custom: added
      this.errors[attribute] = castArray('') // NOTE: custom: added
    } else {
      this._errors[attribute] = castArray(errors) // NOTE: custom: added
      this.errors[attribute] = castArray(errors) // NOTE: custom: original
    }
  }
