...
const post = async (req, res) => {
  ...
  try {
    req.body.state_id = 16 // NOTE: mich
    req.body.town_authority_list_json = JSON.stringify(req.body.town_authority_list_json)
    req.body.possible_candidate_list_json = JSON.stringify(req.body.possible_candidate_list_json)
    req.body.town_actor_list_json = JSON.stringify(req.body.town_actor_list_json)
    req.body.group_list_json = JSON.stringify(req.body.group_list_json)
    req.body.advice_list_json = JSON.stringify(req.body.advice_list_json)
    req.body.numerals_list_json = JSON.stringify(req.body.numerals_list_json)
...
const remove = async (req, res) => {
  ...
  try {
    ...
    if (result.error !== null)
      throw result.error
    res.pushBroadcastMessage(req.session.user.area_id, {
      channel: `${ req.routeOptions.config.resource_name }-deleted`,
      data: result.record,
      notification: {
        user_id: req.session.user.id,
        notification_type: 'log',
...
const getByTown = async (req, res) => {
  ...
  try {
    let result = await REPOSITORY.findByTownId({
      record: { id: req.params.id, },
    })
    if (result.error !== null)
      throw result.error
